...
local KINDS = { { "Knife", "칼" }, { "Barrel", "통" }, { "Ghost", "해적" } }
...
for index, entry in ipairs(KINDS) do
	kindButtons[entry[1]] = textButton(kindRow, entry[2] .. " 스킨", UDim2.fromOffset(92, 26), UDim2.fromOffset((index - 1) * 98, 0))
end
...
local function drawShop()
	...
	for _, entry in ipairs(state.catalog[currentKind] or {}) do
		...
		label(row, entry.name, UDim2.fromOffset(190, 20), UDim2.fromOffset(12, 8), 15,
			entry.rarityColor or PALETTE.Cream)
		...
		label(row, ("%s  ·  %s"):format(entry.rarityLabel, detail),
			UDim2.fromOffset(240, 18), UDim2.fromOffset(12, 28), 12, PALETTE.Dim)
...
local function drawSabotage()
	...
	for _, item in ipairs(sabotageState.items) do
		...
		local usable = item.ready and selectedTarget ~= nil
		local buy = textButton(row, item.ready and ("R$ %d"):format(item.robux) or "준비 중",
		...
		buy.Activated:Connect(function()
			if not usable then
				showToast(item.ready and "먼저 상대를 골라 주세요" or "이 아이템은 아직 준비 중입니다", false)
				return
			end
...
local function redraw()
	...
	scroller.Position = UDim2.fromOffset(20, currentTab == "shop" and 100 or 70)
	scroller.Size = UDim2.new(1, -40, 1, currentTab == "shop" and -120 or -90)
