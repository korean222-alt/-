...
function LobbyBuilder:_buildShowcase()
	...
	for _, zone in ipairs(SHOWCASE.Zones) do
		...
		if list and #list > 0 then
			...
			local count = #list
			local span = (count - 1) * SHOWCASE.Spacing
			...
			for index, skin in ipairs(list) do
				local x = startX + (index - 1) * SHOWCASE.Spacing
				local base = CFrame.new(x, 3.35, SHOWCASE.Z) -- 받침대 윗면

				local pedestal = makeCylinder(folder, "Pedestal", 6.4, 2.6,
					CFrame.new(x, 1.8, SHOWCASE.Z), Color3.fromRGB(38, 28, 22), Enum.Material.Slate)
				...
				makeCylinder(folder, "Rim", 6.7, 0.25,
					CFrame.new(x, 3.2, SHOWCASE.Z), rarity.color, Enum.Material.Neon)
				...
				makeSign(folder, "Plate",
					CFrame.new(x, 3.9, SHOWCASE.Z + 3.3),
