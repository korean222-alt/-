...
function CodeService:Redeem(player, text, now)
	...
	-- Phase 22 : 스킨 선물 코드 (GameConfig.LikeReward 의 파란 철제 드럼)
	local giftName = nil
	if entry.gift == "like" then
		...
		if not skin or skin.id ~= like.Skin then
			return false, "지금은 받을 수 없어요", 0
		end
		if profile.likeClaimed or (profile.owned[like.Kind] and profile.owned[like.Kind][like.Skin]) then
			profile.codes[code] = true
			return false, "이미 받았어요 ✔", 0
		end
		profile.likeClaimed = true
		ProfileService:Grant(player, like.Kind, like.Skin)
		if like.AutoEquip then
			ProfileService:Equip(player, like.Kind, like.Skin)
		end
		player:SetAttribute("LikeClaimed", true)
		giftName = skin.name
	end
	...
	if giftName then
		return true, ("🎁 「%s」 받았어요! (장착됨)"):format(giftName), coins, giftName
	end
