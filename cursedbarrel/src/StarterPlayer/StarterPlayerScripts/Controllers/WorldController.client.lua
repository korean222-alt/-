...
-- Phase 21 : 앉은 테이블 · 관전 중인 테이블에서 판이 진행 중인가 (예전에는 위 알림판이 보이는지로 봤는데,
--   해적이 나오는 동안에는 알림판을 숨기므로 UIKit.matchTable 로 직접 본다)
local UIKit = require(package.Shared:WaitForChild("UIKit"))
local function inTableMatch()
	local _, live = UIKit.matchTable()
	return live
end
...
local function bolt()
	...
	task.delay(math.clamp(near / 500, 0.2, 1.6), function()
		Sfx.play("Thunder", { pitch = 0.85 + math.random() * 0.3, volume = math.clamp(1 - near / 700, 0.15, 0.55) })
		if near < 260 then
...
Run.Heartbeat:Connect(function(dt)
	...
	-- 번개 (습격 중에는 조금 더 자주)
	-- Phase 21 : "너무 자주 쳐서 시끄럽다" → 18~34초에 한 번(습격 중 10~18초), 게임 중(테이블 · 관전)에는 치지 않는다
	local raid = workspace:GetAttribute("RaidActive") == true
	if (sky.Lightning or 0) > 0.5 and os.clock() > nextBolt then
		nextBolt = os.clock() + (raid and (10 + math.random() * 8) or (18 + math.random() * 16))
		if not stage and not inTableMatch() then
			bolt()
		end
	end
	...
	-- Phase 21 : 게임 중에는 날씨 시계를 숨긴다 (위 알림판 뒤로 "새벽 0:08" 이 비쳐 보였다)
	pill.Visible = not inTableMatch()
