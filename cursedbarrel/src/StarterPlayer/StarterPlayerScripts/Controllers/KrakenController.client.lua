...
local function buildTube(model, radii, detailed)
	...
	-- Phase 17 : Blender 살결 마디(끝이 둥근 캡슐)가 있으면 이웃 마디끼리 겹쳐서 이음새를 덮는다.
	--   예전의 둥근 관절 공 · 원통 배 살(파트)은 쓰지 않는다 → 이어진 한 줄기 다리처럼 보인다.
	local capsule = MeshKit.has("KrakenCapsule")
	tube.capsule = capsule
	-- Phase 17.1 : 가져오기가 180° 돌려 넣은 메시를 되돌리는 돌림 (마디 · 배 살 · 빨판)
	tube.segFix = (capsule and MeshKit.fixOf("SK_Kraken_Capsule")) or (meshed and MeshKit.fixOf("CB_Kraken_Segment")) or CFrame.new()
	tube.bellyFix = capsule and MeshKit.fixOf("SK_Kraken_Capsule") or CFrame.new()
	tube.suckerFix = meshed and MeshKit.fixOf("CB_Kraken_Sucker") or CFrame.new()
	for i = 1, count do
		local u = (i - 1) / (count - 1)
		local r = radii[i]
		...
		if i > 1 then
			...
			local seg = (capsule and meshPiece(model, "Segment", "SK_Kraken_Capsule", Vector3.new(1, d, d), color))
				or (meshed and meshPiece(model, "Segment", "CB_Kraken_Segment", Vector3.new(1, d, d), color))
			...
			if detailed then
				local bd = d * 0.8
				local belly = (capsule and meshPiece(model, "Belly", "SK_Kraken_Capsule", Vector3.new(1, bd, bd), COLORS.Belly:Lerp(color, 0.2)))
					or part(model, "Belly", Enum.PartType.Cylinder, Vector3.new(1, bd, bd), COLORS.Belly:Lerp(color, 0.2))
...
local function placeTube(tube, points, normal, sign, given)
	...
	for i = 1, count do
		...
		if seg then
			...
			-- 캡슐 마디는 둥근 끝(지름만큼)이 이웃 마디와 겹치게 늘린다
			local segLength = tube.capsule and (length + seg.Size.Y * 0.95) or length
			if math.abs(seg.Size.X - segLength) > 0.08 then
				seg.Size = Vector3.new(segLength, seg.Size.Y, seg.Size.Z)
			end
			table.insert(moveParts, seg)
			table.insert(moveFrames, along(a, p) * tube.segFix)
			...
			if belly then
				...
				local bellyLength = tube.capsule and (length + belly.Size.Y * 0.95) or length
				if math.abs(belly.Size.X - bellyLength) > 0.08 then
					belly.Size = Vector3.new(bellyLength, belly.Size.Y, belly.Size.Z)
				end
				table.insert(moveParts, belly)
				table.insert(moveFrames, along(a + off, p + off) * tube.bellyFix)
			end
		end
	end
	for _, sucker in ipairs(tube.suckers) do
		...
		table.insert(moveFrames, facing(at, face) * (sucker.cup and CFrame.new() or tube.suckerFix))
		if sucker.cup then
...
local function buildHead()
	...
	if mantle then
		mantle.Locked = true
		h.low = mantle
		h.lowOffset = CFrame.new(MeshKit.Catalog.Pieces.CB_Kraken_Mantle.center) * MeshKit.fixOf("CB_Kraken_Mantle")
	else
...
ReplicatedStorage.ChildAdded:Connect(function(child)
	if MeshKit.isLibraryName(child.Name) then
		queueRebuild()
	end
end)
