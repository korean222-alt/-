...
-- Phase 21 : 휴대폰에서는 창을 조금 줄이고(0.82배) 화면 아래로 내린다.
--   예전에는 점프 버튼을 피하느라 높이 떠 있어서 창 위의 "한 번 더!" 버튼이 화면 위 알림판 밑에 가려졌다.
--   점프 버튼(오른쪽 아래)과 옆으로 겹치지 않을 만큼 화면이 넓으면 바닥 가까이 붙인다.
...
local function fitPanel()
	...
	-- 가로 휴대폰은 높이가 330~430 이라 높이에 맞춰 0.7 ~ 0.85 배
	local scale = phone and math.clamp(view.Y / 440, 0.7, 0.85) or 1
	panelScale.Scale = scale
	local bottom = -16
	if UserInputService.TouchEnabled then
		-- 창 오른쪽 끝과 점프 버튼(오른쪽 끝에서 약 170px) 사이가 넉넉하면 바닥에, 아니면 점프 버튼 위로
		local sideRoom = (view.X - 460 * scale) / 2
		bottom = sideRoom >= 175 and -10 or -92
	end
	panel.Position = UDim2.new(0.5, 0, 1, bottom)
end
fitPanel()
...
local function refresh()
	...
	-- Phase 21 : 해적이 튀어나오는 동안에는 창을 숨겨서 해적이 가려지지 않게 한다
	gui.Enabled = myTurn and localPlayer:GetAttribute("PirateFocus") ~= true
...
localPlayer:GetAttributeChangedSignal("PirateFocus"):Connect(function()
	refresh()
end)
