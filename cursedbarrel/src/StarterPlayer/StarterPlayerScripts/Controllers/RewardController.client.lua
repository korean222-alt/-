...
local attend = UIKit.window(gui, { name = "Attendance", title = "출석 체크", theme = "purple", icon3D = "Attendance", size = Vector2.new(640, 440) })
...
for index, reward in ipairs(ATTEND.Days) do
	...
	local amountLabel = UIKit.label(cell, { text = Utility.comma(reward.coins), size = UDim2.new(1, -10, 0, 28), position = UDim2.fromOffset(5, 3), textSize = 25, stroke = 3, zIndex = 5 })
	-- Phase 16 : 칸마다 돈 모양 그림 (날이 갈수록 돈이 많아지고 7일째는 보물 상자)
	-- Phase 21 : 돈 그림을 크게 (칸을 거의 꽉 채운다)
	local icon = MoneyIcon.view(cell, DAY_ICONS[index] or "cash", { size = UDim2.new(1, -2, 0, 88), position = UDim2.fromOffset(1, 25), spin = index == #ATTEND.Days })
	UIKit.label(cell, { text = ("%d일"):format(index), size = UDim2.new(1, 0, 0, 26), position = UDim2.new(0, 0, 1, -28), textSize = 22, stroke = 3, zIndex = 5 })
...
local roulette = UIKit.window(gui, { name = "Roulette", title = "행운의 룰렛", theme = "orange", icon3D = "Roulette", size = Vector2.new(660, 460) })
...
local function launcher(name, icon3D, caption, imageId, order, themeName, onClick)
	local b, dot = UIKit.railButton({ name = name, icon3D = icon3D, caption = caption, image = imageId, order = order, theme = themeName, side = "right" })
...
local rouletteButton, rouletteDot = launcher("RouletteButton", "Roulette", "룰렛", tonumber(Release.Images and Release.Images.Roulette) or 0, 4, "orange", function()
...
local codeWin = UIKit.window(gui, { name = "Codes", title = "코드 입력", theme = "teal", icon3D = "Code", size = Vector2.new(500, 300) })
...
local likeWin = UIKit.window(gui, { name = "LikeReward", title = "그룹 가입 보상", theme = "blue", size = Vector2.new(600, 360) })
...
local function bindLikePrompt(prompt)
	...
	-- Phase 22 : 받침대를 누르면 코드 창이 열리고 love 가 적혀 있다 (코드 쓰는 법도 같이 배운다)
	prompt.Triggered:Connect(function()
		codeResult.Text = "love 를 적고 「확인」!"
		codeResult.TextColor3 = COLORS.Gold
		codeBox.Text = "love"
		codeWin.open()
	end)
end
...
launcher("CodeButton", "Code", "코드", 0, 5, "teal", function()
...
rewardCue.OnClientEvent:Connect(function(kind, ok, result)
	if kind == "attend" then
		if ok and typeof(result) == "table" then
			...
			-- Phase 21 : "획득!" (빛살 · 반짝임)
			UIKit.rewardPopup({ text = ("%s 코인"):format(Utility.comma(result.coins)), money = DAY_ICONS[result.day] or "cash" })
			claimButton.Text = "내일 또 와요"
			UIKit.setTheme(claimButton, "grey")
