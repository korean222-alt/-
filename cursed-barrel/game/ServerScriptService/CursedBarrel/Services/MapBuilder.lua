...
local function part(parent, name, size, cframe, color, material)
    ...
    if name=="Plank" or name=="Gangplank" or name=="Bulwark" then instance.CanCollide=true;instance.CanQuery=true end
...
function MapBuilder:_buildSea(parent)
	...
	if deck and not workspace:GetAttribute("ShipLobbyReady") then
...
function MapBuilder:Start()
	...
	local ok, err = pcall(function()
		...
		-- The playable lobby is now the flagship itself (ShipLobbyBuilder).
		-- Keep distant boats/sea, not a second unrelated dock and floating lanterns.
		...
		if not workspace:GetAttribute("ShipLobbyReady") then self:_buildDocks(harbor) end
	end)
